...
describe('log function', () => {
  it('should log an info message with the correct timestamp and icon', () => {
    ...
    const expectedOutput = `ℹ️ ${dayjs().format(DATEHOUR_FORMAT)} - ${message}`;
  ...
  it('should log a warn message with the correct timestamp and icon', () => {
    ...
    const expectedOutput = `⚠️ ${dayjs().format(DATEHOUR_FORMAT)} - ${message}`;

    // Call the log function with "warn" level
    log(message, 'warn');

    // Check if console.log was called with the correct format and icon
    expect(console.log).toHaveBeenCalledWith(expect.stringMatching(new RegExp(`⚠️ .+ - ${message}`)));
    expect(console.log).toHaveBeenCalledWith(expect.stringMatching(expectedOutput));
  });

  it('should log an error message with the correct timestamp and icon', () => {
    const message = 'Test Error Message';
    const expectedOutput = `❌ ${dayjs().format(DATEHOUR_FORMAT)} - ${message}`;

    // Call the log function with "error" level
    log(message, 'error');

    // Check if console.log was called with the correct format and icon
    expect(console.log).toHaveBeenCalledWith(expect.stringMatching(new RegExp(`❌ .+ - ${message}`)));
    expect(console.log).toHaveBeenCalledWith(expect.stringMatching(expectedOutput));
  });
  ...
  it('should log multiple messages at once', () => {
    ...
    const expectedOutput1 = `ℹ️ ${dayjs().format(DATEHOUR_FORMAT)} - Message 1`;
    const expectedOutput2 = `ℹ️ ${dayjs().format(DATEHOUR_FORMAT)} - Message 2`;

    // Call the log function with multiple messages
    log(messages);

    // Check if console.log was called for each message
    expect(console.log).toHaveBeenCalledWith(expect.stringMatching(new RegExp(`ℹ️ .+ - Message 1`)));
    expect(console.log).toHaveBeenCalledWith(expect.stringMatching(expectedOutput1));
    expect(console.log).toHaveBeenCalledWith(expect.stringMatching(new RegExp(`ℹ️ .+ - Message 2`)));
